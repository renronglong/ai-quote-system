    ...
    <div className="space-y-6">
              ...
              <TableBody>
                {filteredTasks.map((task) => (
                  <TableRow key={task.id}>
                    ...
                    <TableCell>{task.product_count || 0}</TableCell>
                    <TableCell>
                      {task.total_credits ? `¥${parseFloat(task.total_credits).toFixed(2)}` : '-'}
                    </TableCell>
      ...
      <Dialog open={detailDialogOpen} onOpenChange={setDetailDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>任务详情 - {selectedTask?.task_code}</DialogTitle>
            <DialogDescription>
              {selectedTask && (
          ...
          {selectedTask && (
            <div className="space-y-6 py-4">
              ...
              {/* 上传文件 */}
              {selectedTask.files && (selectedTask.files as TaskFile[]).length > 0 && (
                <div>
                  ...
                  <div className="mt-2 space-y-2">
                    {(selectedTask.files as TaskFile[]).map((file, index) => (
              ...
              {/* 对话记录 */}
              {selectedTask.conversation_log && (selectedTask.conversation_log as Array<{role: string; content: string}>).length > 0 && (
                <div>
                  <Label className="text-gray-500 flex items-center gap-2">
                  ...
                  <div className="mt-2 space-y-2 max-h-48 overflow-y-auto">
                    {(selectedTask.conversation_log as Array<{role: string; content: string}>).slice(-5).map((msg, index) => (
                      <div 
                        key={index} 
              ...
              {selectedTask.admin_notes && (
                <div>
                  <Label className="text-gray-500">管理员备注</Label>
                  <p className="mt-1 p-2 bg-yellow-50 rounded text-sm">
                    {selectedTask.admin_notes}
                  </p>
                </div>
              )}
      ...
      <Dialog open={processingDialogOpen} onOpenChange={setProcessingDialogOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>处理任务 - {selectedTask?.task_code}</DialogTitle>
            <DialogDescription>
              设置任务处理参数和结果
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label htmlFor="product_count">产品数量</Label>
                <Input
                  id="product_count"
                  type="number"
                  min="0"
              ...
              <div>
                <Label htmlFor="model_cost">模型成本 (元)</Label>
                <Input
                  id="model_cost"
                  type="number"
                  min="0"
            ...
            <div>
              <Label htmlFor="price_multiplier">价格倍数</Label>
              <Input
                id="price_multiplier"
                type="number"
                min="0.1"
            ...
            <div className="p-3 bg-blue-50 rounded-lg">
              <Label className="text-blue-800">计算积分</Label>
            ...
            <div>
              <Label htmlFor="result_summary">处理结果</Label>
              <Textarea
                id="result_summary"
