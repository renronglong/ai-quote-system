...
function calcExtrusion(
  ...
  if (req.mold_cost && req.mold_cost > 0) {
    ...
    const SAFE_METER_WEIGHT_LIMITS: Record<string, number> = {
      '139x55': 1.2, '139x65': 1.71, '139x90': 2.21, '139x110': 2.21,
      '158x120': 2.92, '158x130': 2.92,
      '178x130': 3.46,
      '198x130': 4.24, '198x160': 4.24,
      '218x160': 5.37,
      '248x160': 6.88, '248x190': 6.88,
      '278x190': 7.71, '278x230': 7.71,
      '297x190': 9.84, '297x230': 9.84, '297x260': 9.84,
      '338x190': 13.33, '338x230': 13.33, '338x260': 13.33,
      '397x230': 16, '397x260': 16, '397x360': 16,
    };

    // 每个规格的最大米重上限（=最厚档位的值）
    const MAX_METER_WEIGHT_BY_SIZE: Record<number, number> = {
      139: 2.21, 158: 2.92, 178: 3.46, 198: 4.24, 218: 5.37,
      248: 6.88, 278: 7.71, 297: 9.84, 338: 13.33, 397: 16,
    };
