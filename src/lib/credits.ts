...
export const REFERRAL_BONUS_CREDITS = 100;    // 邀请好友注册，邀请人+100
 ...
 * @returns 新余额；consume 余额不足时返回 null（不扣减）
 */
export async function changeCredits(
  ...
  await client.from('credit_balances').upsert(updateData, { onConflict: 'user_id' });
